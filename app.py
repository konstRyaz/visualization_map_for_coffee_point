...
def get_gradient_color(score, min_score, max_score):
    if max_score == min_score: # Избегаем деления на ноль, если все скоры одинаковые
        normalized_score = 0.5
    else:
        normalized_score = (score - min_score) / (max_score - min_score)

    # Убедимся, что нормализованный скор находится в диапазоне [0, 1]
...
if 'fixed_gradient' not in st.session_state: st.session_state.fixed_gradient = False # НОВОЕ: для фиксированного градиента
...
if settings_mode == "Расширенный":
    ...
    st.session_state.min_score_filter = st.sidebar.slider("Показывать POI с минимальным скором:", 0.0, 1.0, st.session_state.min_score_filter, 0.05, key='min_score_slider')
    
    # НОВОЕ: Чекбокс для фиксированного градиента
    st.session_state.fixed_gradient = st.sidebar.checkbox("Фиксировать цветовой градиент (0.0-1.0)", st.session_state.fixed_gradient, key='fixed_gradient_checkbox')


    if not st.session_state.show_heatmap: # Настройки для кругов
        st.sidebar.markdown("---")
...
with st.sidebar.expander("📖 Руководство пользователя"):
    ...
    * **Фиксировать цветовой градиент (0.0-1.0):**
        * **Тип:** Чекбокс.
        * **Назначение:** При включении (отмечен) цвет кругов будет рассчитываться исходя из полного диапазона возможных скоров от 0.0 (красный) до 1.0 (зеленый), независимо от применяемого фильтра "Показывать POI с минимальным скором". Это обеспечивает постоянство цветовой шкалы и позволяет сравнивать скоры между собой напрямую, без перекалибровки градиента под отфильтрованный набор данных. Если отключен, градиент цвета кругов будет адаптироваться к минимальному и максимальному скорам *отображаемых* POI.
...
def get_folium_map(current_city_selection, current_show_heatmap, current_min_score_filter,
                   ...
                   current_fixed_gradient, # НОВОЕ: Передаем состояние фиксированного градиента
                   pois_df):
    ...
    if not filtered_pois_df.empty:
        # Диапазон скоров для отображаемых POI
        min_displayed_score = filtered_pois_df['score'].min()
        max_displayed_score = filtered_pois_df['score'].max()
        
        # Определение диапазона для цветового градиента кругов
        color_min_score = 0.0
        color_max_score = 1.0
        if not current_fixed_gradient: # Если градиент НЕ фиксированный, используем диапазон отображаемых POI
            color_min_score = min_displayed_score
            color_max_score = max_displayed_score
            if color_min_score == color_max_score: # Избегаем деления на ноль, если все отфильтрованные скоры одинаковые
                color_min_score = 0.0
                color_max_score = 1.0 # Используем полный диапазон, чтобы цвет был хоть каким-то
        ...
        if current_show_heatmap:
            ...
            for index, row in filtered_pois_df.iterrows():
                ...
                score = row['score']
                # Передаем выбранный диапазон для цвета круга
...
# Отображаем сообщение о количестве точек после загрузки
...
m = get_folium_map(st.session_state.city_selection,
                   ...
                   st.session_state.fixed_gradient, # НОВОЕ: передаем состояние чекбокса
                   current_pois_df)
...
if not current_pois_df[current_pois_df['score'] >= st.session_state.min_score_filter].empty:
    if st.session_state.show_heatmap:
        ...
        if st.session_state.fixed_gradient:
            st.markdown("*Цветовой градиент кругов зафиксирован от 0.0 (красный) до 1.0 (зеленый).*")
        else:
